...
def extract_patch(img: np.ndarray, kp: cv2.KeyPoint, patch_size: int) -> np.ndarray:
    ...
    M[:, :2] = scale * np.array([[cos_theta, -sin_theta], [sin_theta, cos_theta]])
    ...
    h, w = img.shape[:2]
    if (
        transformed_corners[:, 0].min() < 0
        or transformed_corners[:, 0].max() >= w
        or transformed_corners[:, 1].min() < 0
        or transformed_corners[:, 1].max() >= h
    ):
        return None

    M_inverse = cv2.invertAffineTransform(M)
    patch = cv2.warpAffine(
        img, M_inverse, (patch_size, patch_size), flags=cv2.INTER_LINEAR
    )
    return patch


def compute_homography(
    img1: np.ndarray, img2: np.ndarray, local_descriptor_model=None
) -> np.ndarray:
    """
    ...
    for sigma in [1.6, 2.5, 3.5, 1.4, 1.2]:
        ...
        if len(src_pts) > 30:
            H, mask = cv2.findHomography(
                src_pts, dst_pts, cv2.RANSAC, 15.0, confidence=0.9999
            )
        else:
            H = None
        ...
        t4 = time.time()
        logging.info(
            f"MATCH ransac_matches: {mask.sum()}, good_matches: {len(good_matches)}, kp1: {len(kp1)}, kp2: {len(kp2)} in {t2 - t1:.2f}, {t3 - t1:.2f}, {t4 - t1:.2f} s"
        )
        if mask.sum() > 30:
            break
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Extract homography patches from two images."
    )
    parser.add_argument(
        "--images",
        nargs="+",
    ...
    parser.add_argument(
        "--patch-size", type=int, default=80, help="Size of the extracted patches."
    )
    parser.add_argument(
        "--output-path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="patch",
        help="Prefix for the output patch filenames.",
    )
    parser.add_argument(
        "--output-lmdb", type=str, help="Path to save the extracted patches."
    )
    parser.add_argument(
        "--grid-step", type=int, default=32, help="Step size for the grid points."
    )
    ...
    reference_image = cv2.resize(
        reference_image,
        (reference_image.shape[1] // 2, reference_image.shape[0] // 2),
    ...
    grid_points = np.array(
        [
            [x, y]
            for x in range(0, reference_image.shape[1], args.grid_step)
            for y in range(0, reference_image.shape[0], args.grid_step)
        ],
        dtype=np.float32,
    )
    ...
    for image_id, img_path in enumerate(args.images[1:]):
        ...
        img2 = cv2.resize(
            img2, (img2.shape[1] // 2, img2.shape[0] // 2), interpolation=cv2.INTER_AREA
        )
        ...
        of_images = [
            cv2.resize(
                img, (int(img.shape[1] * of_scale), int(img.shape[0] * of_scale))
            )
            for img in of_images
        ]

        img = np.stack(of_images, 0)
        img_torch = preprocess(
            torch.from_numpy(np.transpose(img, (0, 3, 1, 2)) / 255.0)
        ).to(DEVICE)
        ...
        flow_x = cv2.resize(
            flow_x, (reference_image.shape[1], reference_image.shape[0])
        )
        flow_y = cv2.resize(
            flow_y, (reference_image.shape[1], reference_image.shape[0])
        )

        map_x = (
            np.arange(0, reference_image.shape[1], 1)[np.newaxis, :] + flow_x
        ).astype(np.float32)
        map_y = (
            np.arange(0, reference_image.shape[0], 1)[:, np.newaxis] + flow_y
        ).astype(np.float32)
        img2_warped_of = cv2.remap(
            img2_warped, map_x, map_y, interpolation=cv2.INTER_LINEAR
        )
        ...
        grid_points_2 = np.stack(
            [
                ...
                map_y[grid_points[:, 1].astype(int), grid_points[:, 0].astype(int)],
            ],
            axis=1,
        )
        H_inv = np.linalg.inv(H)
        grid_points_2 = cv2.perspectiveTransform(
            grid_points_2[np.newaxis, :, :], H_inv
        )[0]
        ...
        if args.output_lmdb:
            ...
            for i, (x, y) in enumerate(grid_points_2):
                ...
                patch_img = cv2.imencode(
                    ".jpg", patch, [int(cv2.IMWRITE_JPEG_QUALITY), 98]
                )[1].tobytes()
        ...
        logging.info(
            f"Accumulated image {image_id + 1}/{len(args.images) - 1}: {img_path}"
        )
